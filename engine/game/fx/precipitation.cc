...
Precipitation::Precipitation()
{
   ...
   {
      F32 y1 = (F32) v / DROPS_PER_SIDE;
      F32 y2 = (F32)(v+1) / DROPS_PER_SIDE;
      for (U32 u = 0; u < DROPS_PER_SIDE; u++)
      {
         F32 x1 = (F32) u / DROPS_PER_SIDE;
         F32 x2 = (F32)(u+1) / DROPS_PER_SIDE;
   ...
   {
      F32 y1 = (F32) v / FRAMES_PER_SIDE;
      F32 y2 = (F32)(v+1) / FRAMES_PER_SIDE;
      for (U32 u = 0; u < FRAMES_PER_SIDE; u++)
      {
         F32 x1 = (F32) u / FRAMES_PER_SIDE;
         F32 x2 = (F32)(u+1) / FRAMES_PER_SIDE;
...
{
   AssertFatal(isClientObject(), "Precipitation::fillDropList - server side only!");
...
void Precipitation::processTick(const Move *)
{
   ...
   // We use the camera object as this is view dependent behavior, so
   // let's get a handle to it.
