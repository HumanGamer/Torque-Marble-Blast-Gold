...
class Stream {
  ...
  public:
   ...
   /// Write a debug guard value.
   ///
   /// This helper function, used in conjunction with checkGuardValue(), lets you
   /// easily insert marker values into a stream. You can then check for their
   /// presence, and assert if they are not found.
   ///
   /// @see checkGuardValue()
   void writeGuardValue(U32 val)
   {
#ifdef TORQUE_DEBUG
      write(val);
#endif
   ...
   /// Check for the presence of a debug guard value, and assert if it is not
   /// found.
   ///
   /// This helper function, used in conjunction with writeGuardValue(), is
   /// a powerful mechanism for ensuring that your reads and writes are aligned.
   ///
   /// @code
   /// // Write code:
   /// bstream->write(someVal);
   /// bstream->writeGuardValue(0xDEADBEEF);
   /// bstream->write(moreVal);
   ///
   /// // Later on, read code:
   /// bstream->read(&someVal);
   /// bstream->checkGuardValue(0xDEADBEEF, "Failed to find guard!");
   /// bstream->read(&moreVal);
   /// @endcode
   ///
   /// If the guard fails to check, then you know that the read and write code
   /// isn't matching up properly. Judicious use of guards can allow you to 
   /// easily identify the point of failure.
   ///
   /// @note Be careful when you use these - they are only active in a debug build,
   ///       so writing to files with these methods in debug mode can yield broken
   ///       data. These are best used only when necessary and removed/commented out
   ///       when the problem is fixed.
   void checkGuardValue(U32 val, const char * err)
   {
#ifdef TORQUE_DEBUG
      U32 g = 0;
      read(&g);

      AssertFatal(g==val, err);
#endif
   }
